...
int main() {
	...
	//ICGN 2nd
	ICGN2D2* icgn2 = new ICGN2D2(subset_radius_x, subset_radius_y, max_deformation_norm, max_iteration);
	icgn2->setImages(ref_img, tar_img);
	icgn2->setThreadNumber(CPU_thread_number);
	icgn2->prepare();
...
#pragma omp parallel for
	for (int i = 0; i < POI_queue.size(); ++i) {
		feature_affine->compute(&POI_queue[i]);
