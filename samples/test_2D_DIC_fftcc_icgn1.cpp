...
int main() {
	...
	//set POIs
	Point2D upper_left_point(60, 30);
	...
	//FFTCC
	FFTCC2D* fftcc = new FFTCC2D(subset_radius_x, subset_radius_y);
	fftcc->setImages(ref_img, tar_img);
	fftcc->setThreadNumber(CPU_thread_number);
	fftcc->prepare();

	//ICGN 1st
	ICGN2D1* icgn1 = new ICGN2D1(subset_radius_x, subset_radius_y, max_deformation_norm, max_iteration);
	icgn1->setImages(ref_img, tar_img);
	icgn1->setThreadNumber(CPU_thread_number);
	icgn1->prepare();

	//calculate displacements
#pragma omp parallel for
	for (int i = 0; i < POI_queue.size(); ++i) {
		fftcc->compute(&POI_queue[i]);
		icgn1->compute(&POI_queue[i]);
		//icgn2->compute(&POI_queue[i]);
	}
