...
namespace opencorr
{
	...
	public:
		...
		static ICLM2D1_* allocate(int subset_radius_x, int subset_radius_y);
		static void release(ICLM2D1_* instance);
		static void update(ICLM2D1_* instance, int subset_radius_x, int subset_radius_y);
	};
	...
	private:
		...
		std::vector<ICLM2D1_*> instance_pool; //pool of instances for multi-thread processing
		ICLM2D1_* getInstance(int tid); //get an instance according to the number of current thread id
	...
	public:
		...
		static ICLM2D2_* allocate(int subset_radius_x, int subset_radius_y);
		static void release(ICLM2D2_* instance);
		static void update(ICLM2D2_* instance, int subset_radius_x, int subset_radius_y);
	};
	...
	private:
		...
		std::vector<ICLM2D2_*> instance_pool;
		ICLM2D2_* getInstance(int tid);
